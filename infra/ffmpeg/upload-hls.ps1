...
Write-Host "-> Uploading hls/$Slug to R2 $R2Bucket as Videos/films/$Slug/" -ForegroundColor Cyan
...
Get-ChildItem -Path $Out -Recurse -File | ForEach-Object {
  ...
  Write-Host "  $key"
  $file = $_.FullName
  & wrangler r2 object put "$R2Bucket/$key" --file="$file" --content-type="$ct" --remote
...
Write-Host "OK https://<R2_PUBLIC_HOST>/Videos/films/$Slug/master.m3u8" -ForegroundColor Green
