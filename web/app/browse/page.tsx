...
export default function BrowsePage() {
  ...
  // Group by genre for carousel view
    ...
    <div className="min-h-screen bg-birgen-black">
      ...
      <div className="pt-24 pb-6 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mt-5 flex-wrap gap-3">
          {/* Genre tabs */}
          <div className="flex gap-2 overflow-x-auto pb-1 scroll-container flex-1">
            {GENRES.map((g) => (
              <button
                key={g}
      ...
      {viewMode === 'carousel' && activeGenre === 'All' ? (
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
            {loading
              ? Array.from({ length: 24 }).map((_, i) => <MovieCardSkeleton key={i} size="sm" />)
              : filtered.map((movie) => (
                  <MovieCard
                    key={movie.movieId}
                    movie={movie}
